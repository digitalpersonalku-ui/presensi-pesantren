...
type Step = 'lokasi' | 'wajah' | 'selesai'

export default function AbsensiWajahPage() {
  const { data: session } = useSession()
  const router = useRouter()
  // TESTING: Skip GPS validation for now
  const [step, setStep] = useState<Step>('wajah')
  ...
  const handleWajahDeteksi = async (descriptor: Float32Array) => {
    if (!lokasi || loading) return
    setLoading(true)
    try {
      const res = await fetch('/api/absensi/wajah', {
        method: 'POST',
      ...
      setHasil({ sukses: res.ok, pesan: data.pesan || (res.ok ? 'Absensi berhasil' : 'Absensi gagal') })
      setStep('selesai')
    } catch {
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-md mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold text-gray-800">Absensi Wajah</h1>
        </div>

        <div className="flex gap-2 mb-6">
          {(['lokasi', 'wajah', 'selesai'] as Step[]).map((s, i) => (
            <div key={s} className={`flex-1 h-1.5 rounded-full ${step === s || (i < (['lokasi','wajah','selesai'] as Step[]).indexOf(step)) ? 'bg-green-600' : 'bg-gray-200'}`} />
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          {step === 'wajah' && (
            <div className="space-y-4">
              <h2 className="font-semibold text-gray-800 text-center">Scan Wajah</h2>
              <FaceCapture mode="verify" onFaceDetected={handleWajahDeteksi} />
              {loading && <p className="text-center text-sm text-gray-500">Menyimpan absensi...</p>}
            </div>
          )}
